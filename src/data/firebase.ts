...
try {
  const configValue = import.meta.env.VITE_APP_FIREBASE_CONFIG;
  if (configValue && configValue.trim()) {
    FIREBASE_CONFIG = JSON.parse(configValue);
    isFirebaseEnabled = true;
  } else {
    console.info("Firebase is disabled (no config provided)");
    FIREBASE_CONFIG = {};
  }
} catch (error: any) {
  console.warn(
...
export const saveFilesToFirebase = async ({
  ...
  if (!isFirebaseEnabled) {
    console.info("Firebase is disabled, skipping file upload");
